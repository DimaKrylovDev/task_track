...
# Предварительный расчёт фактического переноса с трёх шардов на четыре
python -m app.cli.migrate_to_four_shards --strategy modulo --dry-run

# Физический перенос записей с трёх шардов на четыре
# На время выполнения запись пользователей должна быть остановлена.
python -m app.cli.migrate_to_four_shards --strategy modulo
```

Стратегия миграции должна совпадать со стратегией, использованной при `seed`.
Скрипт записывает каждую переносимую строку на новый узел через UPSERT и только
после успешной записи удаляет старую копию. Повторный запуск безопасен.
